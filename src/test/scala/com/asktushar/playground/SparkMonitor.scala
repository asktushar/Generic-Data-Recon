...
/**
 * A class representing a statistics about a sparkSession. Currently, only number of spark jobs
 * submitted and its stages are being tracked.
 */
...
class SparkMonitor extends SparkListener {
  ...
  override def onJobStart(jobStart: SparkListenerJobStart) {
    ...
    println(
      s"Job started with ${jobStart.stageInfos.size} stages: $jobStart " +
  ...
  override def onStageCompleted(stageCompleted: SparkListenerStageCompleted): Unit = {
    ...
    println(
      s"Stage ${stageCompleted.stageInfo.stageId} completed with " +
  ...
  /**
   * @param testFun
   *   thunk to run with com.asktushar.playground.SparkSessionStats as an argument. Provides a
   *   monitoring session where the stats are being reset at the beginning
   */
